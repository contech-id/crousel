  ...
  return (
    <AppShell>
      <section className="mx-auto grid min-h-[calc(100vh-5rem)] max-w-[90rem] items-center gap-10 px-4 py-12 sm:px-6 lg:grid-cols-[0.9fr_1.1fr] lg:px-8">
        <div className="hidden rounded-3xl bg-foreground p-10 text-background lg:block">
          <p className="text-xs font-bold uppercase tracking-[0.22em] text-background/60">Selamat datang kembali</p>
          <h1 className="mt-5 text-5xl font-black tracking-tight">Langkah nyaman dimulai dari sini.</h1>
          <p className="mt-5 max-w-sm leading-7 text-background/65">
            Masuk untuk melanjutkan belanja dan mengelola pesanan Crousel-mu.
          </p>
        </div>
        <div className="mx-auto w-full max-w-md">
          ...
          <form onSubmit={handleSubmit} className="mt-8 space-y-5">
            ...
            <Button type="submit" variant="secondary" size="lg" className="w-full rounded-full">
              Masuk <ArrowRight aria-hidden="true" className="size-4" />
            </Button>
          </form>
          <p className="mt-6 text-center text-sm text-muted-foreground">
            Belum punya akun?{" "}
            <a href="/daftar" className="font-semibold text-foreground hover:underline">
              Daftar sekarang
            </a>
          </p>
          <div className="mt-8 flex justify-center gap-4 text-xs text-muted-foreground">
            <span className="inline-flex items-center gap-1">
              <Phone aria-hidden="true" className="size-3.5" /> Aman dan praktis
            </span>
            <span className="inline-flex items-center gap-1">
              <LockKeyhole aria-hidden="true" className="size-3.5" /> Data terlindungi
            </span>
          </div>
        </div>
      </section>
